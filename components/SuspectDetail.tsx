...
export default function SuspectDetail({
  ...
  return (
    <div className="dossier-page">
      ...
      {/* «В розыске» (Дело №9704) — псевдоним/поддельный документ, лично не
          допрашивается, физически недостижим (см. wanted в state/route.ts,
          посчитан из role "В РОЗЫСКЕ..."). Заменяет протокол допроса,
          "Спросить про..." и "Предъявить улику" целиком одним статусом —
          иначе "Спросить про..." (не завязана на fixed_questions/is_suspect)
          всё равно позволила бы полноценно опросить такого персонажа. */}
      {suspect.wanted ? (
        <div className="dossier-card">
          <p className="dossier-wanted-notice">
            Разыскивается. Актуальное местонахождение неизвестно.
          </p>
        </div>
      ) : (
        <>
          {/* Протокол допроса — только у подозреваемых/свидетелей. Экспертов не
              допрашивают: ни один вопрос эксперта не выдаёт улику (grants_evidence_id
              пуст везде), granted_by_character_id на evidence ни на одного эксперта
              не ссылается — секцию можно убрать чисто в рендере, без последствий
              для данных. */}
          {!suspect.is_expert && (
